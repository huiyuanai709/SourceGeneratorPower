...
");
        ...
        private string ProcessInterface(ITypeSymbol typeSymbol)
        {
...
{{
    ...
    {{
        ...
        private List<string> GenerateUsing(ITypeSymbol typeSymbol)
        {
        ...
        private string GenerateMethods(ITypeSymbol typeSymbol)
        {
            var source = new StringBuilder();
            ...
            foreach (var member in typeSymbol.GetMembers())
            {
                ...
                source.AppendLine(GenerateOtherMethod(typeSymbol, methodSymbol, httpClientName, httpMethod, requestUri));
            }
            return source.ToString();
        }

        private string GenerateGetMethod(ITypeSymbol typeSymbol, IMethodSymbol methodSymbol, string httpClientName, string requestUri)
        {
            ...
            source.AppendLine($@"return (await response.Content.ReadFromJsonAsync<{returnType}>(cancellationToken: {cancellationToken})!)!;");
            source.AppendLine("}");
            return source.ToString();
        }
        
        private string GenerateDeleteMethod(ITypeSymbol typeSymbol, IMethodSymbol methodSymbol, string httpClientName, string httpMethod, string requestUri)
        {
            var returnType = (methodSymbol.ReturnType as INamedTypeSymbol).TypeArguments[0].ToDisplayString();
            var cancellationToken = methodSymbol.Parameters.Last().Name;
            var source = GenerateHttpClient(typeSymbol, methodSymbol, httpClientName);
            source.AppendLine($@"var response = await httpClient.{httpMethod}Async($""{requestUri}"", {cancellationToken});");
            source.AppendLine("response!.EnsureSuccessStatusCode();");
            source.AppendLine($@"return (await response.Content.ReadFromJsonAsync<{returnType}>(cancellationToken: {cancellationToken})!)!;");
            source.AppendLine("}");
            return source.ToString();
        ...
        private string GenerateOtherMethod(ITypeSymbol typeSymbol, IMethodSymbol methodSymbol, string httpClientName, string httpMethod, string requestUri)
        {
            var returnType = (methodSymbol.ReturnType as INamedTypeSymbol).TypeArguments[0].ToDisplayString();
            var cancellationToken = methodSymbol.Parameters.Last().Name;
            var source = GenerateHttpClient(typeSymbol, methodSymbol, httpClientName);
            source.AppendLine($@"var jsonContent = JsonContent.Create({methodSymbol.Parameters[methodSymbol.Parameters.Length - 2].Name}, new MediaTypeHeaderValue(""application/json""){{CharSet = ""UTF-8""}});");
        ...
        private StringBuilder GenerateHttpClient(ITypeSymbol typeSymbol, IMethodSymbol methodSymbol, string httpClientName)
        {
            var source = new StringBuilder($@"
public async {methodSymbol.ReturnType} {methodSymbol.Name}({string.Join(", ", methodSymbol.Parameters.Select(p => $"{p.Type} {p.Name}"))})");
        ...
        private Tuple<string, string> GetHttpMethod(IMethodSymbol methodSymbol)
        {
            ...
            {
                return new Tuple<string, string>("Delete", httpDeleteAttribute.ConstructorArguments[0].Value!.ToString());
            }
        ...
        private string GenerateConstructor(ITypeSymbol typeSymbol)
        {
            var className = typeSymbol.Name.Substring(1);
